...
import {Menu, X, BookCheck, BookA, Key, Power} from 'lucide-react'
...
function NavBar() {
  ...
  return (
    ...
    <div className="p-3 w-full relative">
      <nav className="backdrop-blur-sm bg-white/4 rounded-xl border w-full relative z-50 border-white">
        <div className="flex items-center  justify-between px-4 md:px-6 py-4">
          ...
          <div className="flex items-center">
            ...
            <button
              ...
              className="md:hidden text-white mr-4 p-2 hover:bg-white/10 rounded-lg transition"
            >
              {isMobileMenuOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
            </button>

            {/* Navigation Links - Desktop */}
            <div className="hidden md:flex items-center text-white gap-1">
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden border-t border-white/20 px-4 py-3">
            <div className="flex flex-col space-y-2 text-white">
              ...
              {userToken && !adminToken && (
                <a onClick={() => navigateBrowser("purchased")} className="px-4 py-2 rounded-lg border hover:text-gray-400 hover:cursor-pointer font-mono transition">
                  Purchased
                </a>
              )}
              {!userToken && adminToken && (
                <a onClick={() => navigateBrowser("addCourse")} className="px-4 py-2 border rounded-lg hover:text-gray-400 hover:cursor-pointer font-mono transition">
                  Add Course
                </a>
              )}
              <a href="#" className="px-4 py-2 rounded-lg border hover:text-gray-400 font-mono transition">
                Profile
              </a>
              <a href="#" className="px-4 py-2 rounded-lg border hover:text-gray-500 font-mono transition">
                Help
              </a>
            </div>
          </div>
